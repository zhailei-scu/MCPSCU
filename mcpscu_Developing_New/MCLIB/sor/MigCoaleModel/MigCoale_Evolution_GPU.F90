module MIGCOALE_EVOLUTION_GPU
  ...
  subroutine WalkOneStep(Host_Boxes,Host_SimuCtrlParam,Dev_Boxes,Dev_MigCoaleGVars,Record,TSTEP,CapCal_Dev)
    ...
    ASSOCIATE(Dev_ClusterInfo_GPU=>Dev_Boxes%dm_ClusterInfo_GPU,Dev_DiffusorMap=>Dev_Boxes%dm_DiffusorTypesMap,Dev_Rand=>Dev_MigCoaleGVars%dm_MigCoale_RandDev)
        if(Host_SimuCtrlParam%UPDATETSTEPSTRATEGY .eq. mp_SelfAdjustlStep_NNDR_LastPassage_Integer) then
            ...
            if(Host_SimuCtrlParam%m_NNDR_Diffusant_Independent_Time .eq. .true.) then

                call WalkOneStep_Kernel_NNDR_LastPassage_Diffusant_Independent_Time<<<blocks,threads>>>(BlockNumEachBox,                        &
                                                                        TotalNC,                                     &
            ...
            if(Host_SimuCtrlParam%UPDATETSTEPSTRATEGY .eq. mp_SelfAdjustlStep_NNDR .and. &
               Host_SimuCtrlParam%m_NNDR_Diffusant_Independent_Time .eq. .true.) then

                call WalkOneStep_Kernel_NNDR_Diffusant_Independent_Time<<<blocks,threads>>>(BlockNumEachBox,  &
                                                            TotalNC,                                     &
                                                            Dev_ClusterInfo_GPU%dm_Clusters,             &
                                                            Dev_Boxes%dm_SEUsedIndexBox,                 &
  ...
  attributes(global) subroutine WalkOneStep_Kernel_NNDR_LastPassage(BlockNumEachBox,TotalNC,Dev_Clusters,Dev_SEUsedIndexBox, &
    ...
    !********************************************************
  attributes(global) subroutine WalkOneStep_Kernel_NNDR_Diffusant_Independent_Time(BlockNumEachBox,TotalNC,Dev_Clusters,Dev_SEUsedIndexBox, &
                                                   Dev_RandArray,Dev_ActiveStatu,NSeeds,Dev_GrainSeeds,Dev_TypesEntities,Dev_SingleAtomsDivideArrays,MinTSteps, &
                                                   Dev_CascadeCent,Dev_ROutAbsorbRadius)
    implicit none
    !---Dummy Vars---
    integer,value::BlockNumEachBox
    ...
    return
  end subroutine WalkOneStep_Kernel_NNDR_Diffusant_Independent_Time

  !********************************************************
  attributes(global) subroutine WalkOneStep_Kernel_NNDR_LastPassage_Diffusant_Independent_Time(BlockNumEachBox,TotalNC,Dev_Clusters,Dev_SEUsedIndexBox, &
                                                                    DevRandRecord,Dev_ActiveStatu,NSeeds,Dev_GrainSeeds,&
                                                                    Dev_TypesEntities,Dev_SingleAtomsDivideArrays,MinTSteps,&
                                                                    LowerLimitLength,LastPassageFactor,&
                                                                    Dev_CascadeCent,Dev_ROutAbsorbRadius)
    implicit none
    !---Dummy Vars---
