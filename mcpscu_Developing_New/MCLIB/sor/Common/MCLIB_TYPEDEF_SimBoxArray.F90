...
module MCLIB_TYPEDEF_SIMULATIONBOXARRAY
  ...
  subroutine PutoutToFile(this,Host_SimuCtrlParam,SimuRecord,hFile)
    ...
    if(associated(SimuRecord%GetUDefReadWriteRecord_List())) then
        ...
        cursorUDefWriteRecordList=>SimuRecord%GetUDefReadWriteRecord_List()
...
!    END DO
  ...
  subroutine Putin_OKMC_OUTCFG_FORMAT18_SimRecord(this,hFile,SimuRecord,TheVersion,LINE)
    ...
    DO While(.true.)
        ...
        select case(KEYWORD(1:LENTRIM(KEYWORD)))
            ...
            case("&UDEFSECTION")
                if(associated(SimuRecord%GetUDefReadWriteRecord_List())) then
                    cursor=>SimuRecord%GetUDefReadWriteRecord_List()

                    Do while(associated(cursor))
                        call cursor%TheReadWriteProc(hFile,SimuRecord,LINE)
                        cursor=>cursor%next
                    End Do
                end if
  ...
  subroutine Putin_MF_OUTCFG_FORMAT18_Distribution(this,Host_SimuCtrlParam,cfgFile,LayersThick,ClustersSampleConcentrate,ClustersSample,SimuRecord,SURDIFPRE_FREE)
    ...
    DO While(.true.)
        ...
        if(TheDiffusorValue%DiffuseDirectionType .eq. p_DiffuseDirection_OneDim) then
            ClustersSample(1,NClustersGroup)%m_DiffCoeff = ClustersSample(1,NClustersGroup)%m_DiffCoeff*1.D0/3.D0        ! All Diffusion coeff would be changed to 3-D formation
        end if
  ...
  subroutine Putin_SPMF_OUTCFG_FORMAT18_Distribution(this,Host_SimuCtrlParam,cfgFile,LayersThick,ClustersSampleConcentrate,ClustersSample,SimuRecord,SURDIFPRE_FREE,SURDIFPRE_INGB)
    ...
    DO While(.true.)
        ...
        if(ClustersSample(ILayer,IGroup)%m_Statu .eq. p_ACTIVEFREE_STATU) then
            ...
            if(TheDiffusorValue%DiffuseDirectionType .eq. p_DiffuseDirection_OneDim) then
                ClustersSample(ILayer,IGroup)%m_DiffCoeff = ClustersSample(ILayer,IGroup)%m_DiffCoeff*1.D0/3.D0        ! All Diffusion coeff would be changed to 3-D formation
            end if
