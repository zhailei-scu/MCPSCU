module MC_ConstructCaptureBox
   ...
   subroutine Generate_Capture(TheCaptureCal,Host_Boxes,Host_SimuCtrlParamList,Record)
        ...
        select case(TheCaptureCal%CaptureGenerateWay)
            ...
            case(CaptureGenWay_Speicaled_Shape)
                ...
                call Generate_Capture_Specialed_Shape(hCfgOut,hOutInfo,TheCaptureCal,Host_Boxes,Host_SimuCtrlParamList,Record)
   ...
   subroutine Generate_Capture_Locally_CentUniform(hCfgOut,hOutInfo,TheCaptureCal,Host_Boxes,Host_SimuCtrlParamList,Record)
        ...
        DO IBox = 2,MultiBox
            ...
            DO IC=ICFROM,ICTO
                ...
                if(TheCaptureCal%SIADistSameBetweenBoxes .eq. .true.) then
                    ...
                    DO I = 1,3
                        Host_Boxes%m_ClustersInfo_CPU%m_Clusters(IC)%m_POS(I) = Host_Boxes%m_ClustersInfo_CPU%m_Clusters(IC)%m_POS(I) + TheCaptureCal%m_CascadeCenter(IBox,I) - TheCaptureCal%m_CascadeCenter(1,I)
                    END DO
    ...
    !*****************************************************
    subroutine Generate_Capture_Specialed_Shape(hCfgOut,hOutInfo,TheCaptureCal,Host_Boxes,Host_SimuCtrlParamList,Record)
        implicit none
        !---Dummy Vars---
        integer,intent(in)::hCfgOut
        integer,intent(in)::hOutInfo
        ...
        DO IBox = 1,1
            ...
            DO ICase = 1,1
                ...
                DO IIC = 1,TheCaptureCal%UDef_NVAC(ICase)
                    ...
                    select case(TheCaptureCal%VACDist_Shape(1:LENTRIM(TheCaptureCal%VACDist_Shape)))
                        case("LINE")

                            Host_Boxes%m_ClustersInfo_CPU%m_Clusters(IC)%m_POS(1) = TheCaptureCal%UDef_CascadeCent(ICase,1)
                            Host_Boxes%m_ClustersInfo_CPU%m_Clusters(IC)%m_POS(2) = TheCaptureCal%UDef_CascadeCent(ICase,2)
                            Host_Boxes%m_ClustersInfo_CPU%m_Clusters(IC)%m_POS(3) = TheCaptureCal%UDef_CascadeCent(ICase,3) + ((-1)**mod(IIC,2))*TheCaptureCal%Shape_Data(1)*(0.5D0 + (IIC-1)/2)

                        case("CYLINDER")

                            RRXY = TheCaptureCal%Shape_Data(1)*DRAND32()
                            DO I = 1,2
                                Vector(I) = DRAND32() - 0.5D0
                                ArrowLen = ArrowLen + Vector(I)*Vector(I)
                            END DO
                            ...
                            highZ = TheCaptureCal%Shape_Data(2)*(DRAND32() - 0.5D0)
                            Host_Boxes%m_ClustersInfo_CPU%m_Clusters(IC)%m_POS(3) = TheCaptureCal%UDef_CascadeCent(ICase,3) + highZ

                        case("ELLIPSOID")

                            maxLength = 2*maxval(TheCaptureCal%Shape_Data)
                            ...
                            DO While(exitFlag .eq. .false.)
                                ...
                                DO I = 1,3
                                    Host_Boxes%m_ClustersInfo_CPU%m_Clusters(IC)%m_POS(I) = TheCaptureCal%UDef_CascadeCent(ICase,I) + maxLength*(DRAND32() - 0.5D0)
                                END DO

                                judgement = 0.D0
                                DO I = 1,3
                                    judgement = judgement + ((Host_Boxes%m_ClustersInfo_CPU%m_Clusters(IC)%m_POS(I) - TheCaptureCal%UDef_CascadeCent(ICase,I))**2)/(TheCaptureCal%Shape_Data(I)**2)
                                END DO

                                if(judgement .LT. 1.D0) then
                                    exitFlag = .true.
                                end if
                            END DO
    ...
    end subroutine Generate_Capture_Specialed_Shape
