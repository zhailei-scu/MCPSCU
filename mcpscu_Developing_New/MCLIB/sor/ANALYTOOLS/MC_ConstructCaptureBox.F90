            ...
            DO ICase = 1,1
                ...
                DO IIC = 1,TheCaptureCal%UDef_NVAC(ICase)
                    ...
                    select case(TheCaptureCal%VACDist_Shape(1:LENTRIM(TheCaptureCal%VACDist_Shape)))
                        case("SPHERE")
                            RR = TheCaptureCal%UDef_Shape_Data(ICase,1)*DRAND32()
                            ArrowLen = 0.D0
                            DO I = 1,3
                                Vector(I) = DRAND32() - 0.5D0
                                ArrowLen = ArrowLen + Vector(I)*Vector(I)
                        ...
                        case("CYLINDER")
                            ...
                            highZ = TheCaptureCal%UDef_Shape_Data(ICase,2)*(DRAND32() - 0.5D0)
                            Host_Boxes%m_ClustersInfo_CPU%m_Clusters(IC)%m_POS(3) = TheCaptureCal%UDef_CascadeCent(ICase,3) + highZ

                        case("ELLIPSOID")

                            maxLength = 2*maxval(TheCaptureCal%UDef_Shape_Data(ICase,1:3))
                            ...
                            DO While(exitFlag .eq. .false.)
                                ...
                                judgement = 0.D0
                                DO I = 1,3
                                    judgement = judgement + ((Host_Boxes%m_ClustersInfo_CPU%m_Clusters(IC)%m_POS(I) - TheCaptureCal%UDef_CascadeCent(ICase,I))**2)/(TheCaptureCal%UDef_Shape_Data(ICase,I)**2)
                                END DO
