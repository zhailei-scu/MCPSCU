module MC_StatisticClusters_Offline
    ...
    !*******************************************************************************
    subroutine StatisticClusters_SIAAndVAC(CascadeControlFile,Host_Boxes,Host_SimuCtrlParam,Record,hFileOutEachBox,hFileOutTotalBox)
        implicit none
        !---Dummy Vars---
        ...
        DO IBox = 1,MultiBox
            ...
            DO IC = ICFROM,ICTO
                ...
                if(Host_Boxes%m_ClustersInfo_CPU%m_Clusters(IC)%m_Statu .eq. p_ABSORBED_STATU) then
                    if(any(Host_Boxes%m_ClustersInfo_CPU%m_Clusters(IC)%m_Atoms(1:p_ATOMS_GROUPS_NUMBER)%m_NA .LT. 0) .or. &
                       sum(Host_Boxes%m_ClustersInfo_CPU%m_Clusters(IC)%m_Atoms(1:p_ATOMS_GROUPS_NUMBER)%m_NA) .GT. 0) then
        ...
        return
    end subroutine StatisticClusters_SIAAndVAC
    ...
    DO IJOB = Host_SimuCtrlParam%STARTJOB,Host_SimuCtrlParam%ENDJOB,Host_SimuCtrlParam%JOBSTEP
        ...
        DO ITSECTION = Host_SimuCtrlParam%STARTTSECTION,Host_SimuCtrlParam%ENDTSECTION,Host_SimuCtrlParam%TSECTIONSTEP
            ...
            DO ICFG = Host_SimuCtrlParam%STARTCFG,Host_SimuCtrlParam%ENDCFG,Host_SimuCtrlParam%CFGSTEP
                ...
                call StatisticClusters_SIAAndVAC(CascadeControlFile,Host_Boxes,Host_SimuCtrlParam,Record,hFileOutEachBox,hFileOutTotalBox)
