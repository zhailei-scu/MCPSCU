module MCLIB_TYPEDEF_SIMULATIONCTRLPARAM
  ...
  subroutine Print_CtrlParameters(this,hFile)
    ...
    DO while(associated(cursor))
        ...
        write(hFile,fmt="('!',A70,'!',2x,1PE10.4)") "The parameter determine when the neighbore list to be updated =",cursor%NEIGHBORUPDATE

        write(hFile,fmt="('!',A70,'!',2x,1PE10.4)") "The cut-off region expand =",cursor%CUTREGIONEXTEND
